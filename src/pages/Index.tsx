  ...
  return (
    <Layout>
      ...
      <section className="relative overflow-hidden bg-gradient-soft">
        ...
        <div className="container-app py-16 lg:py-24 grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6 animate-fade-in-up">
            <span className="inline-flex items-center gap-2 rounded-full bg-primary-soft text-primary px-4 py-1.5 text-sm font-medium">
              {t.hero.badge}
            </span>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight">
              <span className="text-gradient">{t.hero.title}</span>
            </h1>
            <p className="text-lg text-muted-foreground max-w-xl leading-relaxed">{t.hero.subtitle}</p>
            ...
            <div className="grid grid-cols-3 gap-4 pt-8 border-t border-border/50">
              {[['10K+', t.hero.stat1], ['2K+', t.hero.stat2], ['500+', t.hero.stat3]].map(([n, l]) => (
                <div key={l}>
                  <div className="text-2xl lg:text-3xl font-bold text-primary">{n}</div>
                  <div className="text-xs lg:text-sm text-muted-foreground">{l}</div>
                </div>
              ))}
            </div>
          </div>
          <div className="relative animate-fade-in-up" style={{ animationDelay: '120ms' }}>
            <div className="absolute inset-0 bg-gradient-hero rounded-[2rem] blur-2xl opacity-30" />
            <img src={hero} alt="SOUQI bridges Morocco and China" className="relative rounded-[2rem] shadow-pop w-full" />
          </div>
        </div>
      </section>
      ...
      {/* CTA */}
      <section className="container-app py-20">
        <div className="relative overflow-hidden rounded-[2rem] bg-gradient-hero p-10 lg:p-16 text-center shadow-pop">
          ...
          <div className="relative">
            <h2 className="text-3xl lg:text-5xl font-bold text-white mb-4">{t.cta.title}</h2>
            <p className="text-white/85 max-w-2xl mx-auto mb-8 text-lg">{t.cta.subtitle}</p>
